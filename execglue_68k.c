...
static VOID stub_GetCPUInfoPPC(ULONG *regarray)
{
	struct Library *Base = (struct Library *) regarray[REG68K_A6/4];
	struct ExtendedLibrary *ExtLib = (struct ExtendedLibrary *) ((ULONG)Base + Base->lib_PosSize);
	struct ExecGlueIFace *Self = (struct ExecGlueIFace *) ExtLib->MainIFace;

	Self->GetCPUInfo(
		(struct TagItem *)regarray[8]
	);
}
STATIC CONST struct EmuTrap stub_GetCPUInfo = { TRAPINST, TRAPTYPE, (ULONG (*)(ULONG *))stub_GetCPUInfoPPC };
